...
const AI_SESSION_DIR = join(homedir(), '.claude', 'projects') // default session path
...
export class SessionWatcher {
  ...
  start(): void {
    ...
    this.watcher.on('change', (p) => this.onFileChange(p))
    this.watcher.on('add',    (p) => this.onFileChange(p))
  }
